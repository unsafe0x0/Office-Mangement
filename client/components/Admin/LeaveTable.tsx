        ...
        <div className="bg-neutral-800/50 border border-neutral-700/50 rounded-md overflow-hidden w-full">
                ...
                <table className="min-w-full text-sm text-left text-neutral-300">
                    ...
                    <tbody>
                        {leaveData.map((leave, index) => (
                            <tr key={index} className="border-b border-neutral-700/50 hover:bg-neutral-800/70 relative">
                                <td className="px-6 py-4 flex items-center gap-3">
                                    ...
                                    />
                                    <span className="text-neutral-100 font-normal">{leave.employeeName}</span>
                                </td>
                                <td className="px-6 py-4">{leave.startDate}</td>
                                <td className="px-6 py-4">{leave.endDate}</td>
                                <td className="px-6 py-4">{leave.reason}</td>
                                <td className="px-6 py-4">
                                    <div className={`inline-flex items-center gap-1 px-2 py-1 rounded text-xs font-normal ${getStatusStyle(leave.status)}`}>
                                        {leave.status}
                                    </div>
                                </td>
            ...
            {leaveData.length === 0 && (
                <div className="text-center py-12">
                    ...
                    <h3 className="text-lg font-normal text-neutral-300 mb-2">No leave requests found</h3>
                    <p className="text-neutral-400">All employees are currently working</p>
                </div>
            )}
        </div>
    );
