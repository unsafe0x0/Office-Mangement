...
const EditNotificationForm: React.FC<EditNotificationFormProps> = ({ isOpen, onClose, notification }) => {
    ...
    const targetOptions = [
        ...
        { value: "EMPLOYEE", label: "Employees Only", icon: User, description: "Send to employees only" },
        { value: "ADMIN", label: "Admins Only", icon: Shield, description: "Send to admins only" },
    ];
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 h-screen">
            <div className="bg-gradient-to-b from-neutral-900 to-neutral-800 rounded-md p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto border border-neutral-700/50">
                <div className="mb-6">
                    <h2 className="text-2xl font-bold text-neutral-100">Edit Notification</h2>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            {targetOptions.map((option) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`p-4 rounded-md border transition-all duration-200 focus:outline-none ${formData.forWhom === option.value
                                    ...
                                    >
                                        <div className="flex flex-col items-center gap-2">
                                            <IconComponent
                                                size={20}
                                                className={`transition-colors duration-200 ${formData.forWhom === option.value ? "text-blue-400" : "text-neutral-400"
                                                    }`}
                                            />
                                            <span className={`text-sm font-normal transition-colors duration-200 ${formData.forWhom === option.value ? "text-blue-300" : "text-neutral-300"
                                                }`}>
                                                {option.label}
                                            </span>
                                            <span className={`text-sm text-center transition-colors duration-200 ${formData.forWhom === option.value ? "text-blue-400/70" : "text-neutral-400"
                                                }`}>
                                                {option.description}
                                            </span>
                                        </div>
                    ...
                    <div className="bg-neutral-800/30 border border-neutral-600/50 rounded-md p-4">
                        <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1">
                                <h4 className="text-sm font-normal text-neutral-300 mb-1">Preview</h4>
                                <p className="text-sm text-neutral-400">
                                    {formData.message || "Your notification message will appear here..."}
                                </p>
                                <div className="flex items-center gap-2 mt-2 text-sm text-neutral-500">
                                    <span>Target:</span>
                                    <span className="capitalize">{formData.forWhom.toLowerCase()}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col md:flex-row gap-4 pt-4">
                        ...
                        >
                            {updateNotificationMutation.isPending ? "Updating Notification..." : "Update Notification"}
                        </Button>
                        <RedButton
                            type="button"
                            onClick={handleClose}
