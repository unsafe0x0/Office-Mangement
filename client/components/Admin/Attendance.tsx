...
import { Plus, Calendar, User, CheckCircle, XCircle, Clock, Wifi } from "lucide-react";
    ...
    return (
        <div className="p-6">
            <div className="flex justify-between items-center mb-6">
                <div>
                    <h1 className="text-2xl font-semibold text-neutral-100 font-heading">Attendance</h1>
                    <p className="text-neutral-400">Track employee attendance and manage records</p>
                </div>
            ...
            <div className="space-y-6">
                {sortedDates.length > 0 ? (
                    sortedDates.map((date) => (
                        <div key={date} className="bg-neutral-900 rounded-md overflow-hidden">
                            <div className="bg-neutral-800 px-6 py-3 border-b border-neutral-700">
                                ...
                                <p className="text-neutral-400 text-sm">
                                    {groupedAttendance[date].length} record{groupedAttendance[date].length !== 1 ? 's' : ''}
                                </p>
                            </div>
                                ...
                                <table className="min-w-full text-sm text-left text-neutral-300">
                                    ...
                                    <tbody>
                                        {groupedAttendance[date].map((record) => (
                                            <tr key={record.id} className="border-b border-neutral-700 hover:bg-neutral-800/50">
                                                ...
                                                <td className="px-6 py-4">
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <span className={`px-2 py-1 rounded text-sm font-normal ${getStatusStyle(record.status)}`}>
                    ...
                    <div className="bg-neutral-900 rounded-md shadow-lg">
                        <div className="text-center py-12">
                            ...
                            <h3 className="text-lg font-normal text-neutral-300 mb-2">No attendance records found</h3>
                            <p className="text-neutral-400 mb-4">Start tracking attendance by adding the first record</p>
                            <select
                                onChange={(e) => {
                                    const employee = employees.find(emp => emp.id === e.target.value);
                                    if (employee) {
                                ...
                                defaultValue=""
                            >
                                <option value="" disabled>Select Employee to Add Attendance</option>
                                {employees.map((employee) => (
                                    <option key={employee.id} value={employee.id}>
                                        {employee.name}
                                    </option>
