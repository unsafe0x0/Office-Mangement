...
const EmployeeTable = ({ employees }: { employees: Employee[] }) => {
    ...
    const [isEditAttendanceFormOpen, setIsEditAttendanceFormOpen] = useState(false);
    ...
    return (
        <div className="bg-neutral-800/50 border border-neutral-700/50 rounded-md overflow-hidden w-full">
                ...
                <table className="min-w-full text-sm text-left text-neutral-300">
                    ...
                    <tbody>
                        {employees.map((employee, index) => (
                            <tr key={employee.id} className="border-b border-neutral-700/50 hover:bg-neutral-800/50 relative">
                                <td className="px-6 py-4 flex items-center gap-3">
                                    ...
                                    />
                                    <span className="text-neutral-100 font-normal">{employee.name}</span>
                                </td>
                                <td className="px-6 py-4">{employee.position}</td>
                                <td className="px-6 py-4">{employee.department}</td>
                                <td className="px-6 py-4">{employee.email}</td>
                                <td className="px-6 py-4">{employee.dateOfJoining ? new Date(employee.dateOfJoining).toLocaleDateString() : ""}</td>
                                <td className="px-6 py-4">
                                    <div className={`inline-flex items-center px-2 py-1 rounded text-sm font-normal ${getActiveStyle(employee.isActive)}`}>
            ...
            {isEditAttendanceFormOpen && selectedEmployee && selectedEmployee.attendance && selectedEmployee.attendance[0] && (
                <EditAttendanceForm
