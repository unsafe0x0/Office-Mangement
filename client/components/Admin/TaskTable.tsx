...
const getStatusIcon = (status: string) => {
    switch (status) {
        ...
        case "IN_PROGRESS": return <AlertCircle size={16} className="text-white" />;
        case "PENDING": return <Clock size={16} className="text-white" />;
        case "CANCELLED": return <XCircle size={16} className="text-white" />;
        ...
        <div className="bg-neutral-800/50 border border-neutral-700/50 rounded-md overflow-hidden w-full">
                ...
                <table className="min-w-full text-sm text-left text-neutral-300">
                    ...
                    <tbody>
                        {tasks.map((task, index) => (
                            <tr key={task.id} className="border-b border-neutral-700/50 hover:bg-neutral-800/50 relative">
                                <td className="px-6 py-4">
                                    <div className="flex items-start gap-3">
                                        ...
                                        <div className="flex-1">
                                            <h3 className="font-normal text-neutral-100">{task.title}</h3>
                                            {task.description && (
                                ...
                                <td className="px-6 py-4">
                                    <div className={`inline-flex items-center gap-1 px-2 py-1 rounded text-xs font-normal ${getStatusStyle(task.status)}`}>
                                ...
                                <td className="px-6 py-4">
                                    {task.dueDate ? (
                                        ...
                                        <span className="text-neutral-500 text-sm">No due date</span>
                                    )}
                                </td>
                                <td className="px-6 py-4">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-neutral-300 text-sm">
                                            {task.employeeEmails.length} employee{task.employeeEmails.length !== 1 ? 's' : ''}
                                        </span>
                                    </div>
                                </td>
            ...
            {tasks.length === 0 && (
                <div className="text-center py-12">
                    ...
                    <h3 className="text-lg font-normal text-neutral-300 mb-2">No tasks found</h3>
                    <p className="text-neutral-400">Try adjusting your filters or create a new task</p>
                </div>
            )}
        </div>
    );
